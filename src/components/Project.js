...
const Project = ({ projectNumber }) => {
  ...
  useEffect(() => {
        ...
        // setPlusMinus(Math.random() > .4 ? 1 : -1);
  }, [])
  ...
  const variants = {
    ...
    const imgAnim = {
      initial: {
        opacity: 0,
        x: Math.floor(Math.random() * 350 * (Math.random() > .4 ? 1 : -1)),
        y: Math.floor(Math.random() * 120 * (Math.random() > .4 ? 1 : -1)),
      },
      visible: {
        opacity: 1,
        x: 0,
      ...
      <div className="project-content">
        ...
        <motion.div className="img-content" initial="initial" animate="visible" variants={imgAnim} transition={{duration: 1.2}}>
          ...
          <div className="button-container">
            <a href={currentProject.link} target="_blank" rel="noreferrer noopener" className="hover">
